...
public class CustomFormTagMapperImpl extends BaseMapper implements CustomFormTagMapper {
    ...
    @Override
    public String selectTagIdByTagName(Document queryDoc) {
        CustomFormTag customFormTag = this.mongoTemplate.findOne(new BasicQuery(queryDoc), CustomFormTag.class);
        if (customFormTag != null) {
            return customFormTag.getId();
        }
        return null;
    }
}
